...
$sqll="select * from potrawy where kategoria='Zupy' and podkategoria='tradycyjne'";
setcookie('sqll', serialize($sqll), time()+3600);
$zaw1=mysqli_query($link, $sqll);
$sql_pieczywo="select * from pieczywo";
$zaw2=mysqli_query($link, $sql_pieczywo);
$i = 1;
$zmienna=null;
while($s = mysqli_fetch_array($zaw2)){
		$tab[$i] = $s['nazwa'];
		$tabid[$i]=$s['id_pieczywa'];
		$i++;
	}
while($r = mysqli_fetch_array($zaw1)){
	echo "<div id='dania'>
	<img src='dane/".$r['zdjecie']."' align='left'><div id='opis' align='left'>
	".$r['nazwa']." <br>
	...
	Pieczywo: 
	<select name='pieczywo' id='skomponuj'>
	<option value='".$tabid[1]."'>".$tab[1]."</option>
	<option value='".$tabid[2]."'>".$tab[2]."</option>
	<option selected='selected' value='".$zmienna."'>Brak</option>
	</select><br>
...
mysqli_close($link);
?>
<a href="zupy.php"><button value="Wstecz" id="szukajka">Wstecz</button></a>
</center>
</div>
</body>
</html>
