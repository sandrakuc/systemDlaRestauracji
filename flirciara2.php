...
if (isset($_COOKIE['sqll'])) {
	...
	$glu=null;
	foreach($f as $checkv){
		if($checkv=="wegetarianska")
			$weget="czy_wegetarianska='1'";
		if($checkv=="weganska"){
			if($weget==null)
				$vege="czy_weganska='1'";
			else
				$vege="and czy_weganska='1'";
		}
		if($checkv=="bezglutenowa"){
			if($weget==null && $vege==null)
				$glu="czy_bezglutenowa='1'";
			else
				$glu="and czy_bezglutenowa='1'";
		}
	}
	$zapytka = $sqll." where ".$weget.$vege.$glu;
	$zmienna=null;
	...
	while($r = mysqli_fetch_array($zaw1)){
		echo "<div id='zestaw'><img src='dane/".$r['zdjecie']."' align='left'>
